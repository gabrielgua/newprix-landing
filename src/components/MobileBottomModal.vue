    ...
    <FadeTransition>
      <div class="fixed inset-0 z-50  bg-black/50" v-show="show" @click.self="$emit('on-close')">
        <FadeFromBottomTransition>
          <div class="absolute bottom-0 left-0 right-0 min-h-[50dvh] bg-white rounded-t-2xl p-4 space-y-4" v-if="show">
            <header class="flex items-center justify-between">
              <p v-if="title" class="text-text-primary text-lg font-semibold">{{ title }}</p>
              <Logo v-else />
